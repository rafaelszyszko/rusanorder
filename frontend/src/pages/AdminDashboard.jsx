...
const TABS = [
  ...
  { key: "clients", label: "Clientes & Usuários" },
];
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    getAdminDashboardStats().then(setStats).catch((e) => setError(e.message));
  }, []);
  ...
  // Chart data - Monthly orders bar + revenue line
  ...
  const deliveryChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        data: [r.delivered, r.cancelled, r.in_progress],
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: "rgba(30,24,18,0.95)",
        titleColor: "#e8ddd0",
        bodyColor: "#b5a898",
    ...
    <DashboardLayout>
      <div className="container-fluid px-4 py-4 flex-grow-1">
        <div className="module-header">
          <h3 className="module-title">Painel Administrativo</h3>
          <p className="module-subtitle">Observabilidade do sistema</p>
        </div>
        ...
        {stats && (
          <>
            ...
            <div className="row g-3 mb-4">
              {[
                { label: "Usuários ativos", value: stats.totalUsers, sub: `${stats.adminUsers} admin / ${stats.regularUsers} user${stats.inactiveUsers > 0 ? ` / ${stats.inactiveUsers} inativo` : ""}`, icon: "bi-people-fill", iconBg: "rgba(108,117,125,0.15)", iconColor: "#6c757d" },
                { label: "Pedidos hoje", value: stats.ordersToday, icon: "bi-calendar-check-fill", iconBg: "rgba(13,202,240,0.15)", iconColor: "#0dcaf0" },
                { label: "Total de pedidos", value: stats.totalOrders, icon: "bi-clipboard-data-fill", iconBg: "rgba(13,110,253,0.15)", iconColor: "#0d6efd" },
                { label: "Total de clientes", value: stats.totalClients, icon: "bi-building", iconBg: "rgba(111,66,193,0.15)", iconColor: "#6f42c1" },
            ...
            <div className="dashboard-tabs">
              {TABS.map((t) => (
                <button
                  key={t.key}
                  className={`dashboard-tab-btn ${tab === t.key ? "active" : ""}`}
              ...
              <div className="row g-3">
                <div className="col-12 col-lg-7">
                  <div className="chart-card">
                    <div className="chart-card-header">Pedidos e Faturamento por Mês</div>
                ...
                <div className="col-12 col-lg-7">
                  <div className="chart-card">
                    <div className="chart-card-header d-flex justify-content-between align-items-center">
                      ...
                      <button className="btn btn-outline-secondary btn-sm" style={{ fontSize: "0.72rem" }} onClick={() => navigate("/admin/orders")}>Ver todos</button>
                    </div>
                    <div className="chart-card-body">
                        ...
                        <table className="table table-dark table-sm align-middle mb-0">
                          ...
                          <tbody>
                            {(stats.recentOrders || []).map((o) => (
                              <tr key={o.id} role="button" onClick={() => navigate(`/admin/orders/${o.id}`)}>
            ...
            {tab === "clients" && (
              <div className="row g-3">
                <div className="col-12 col-lg-6">
                  <div className="chart-card">
                    <div className="chart-card-header">Top Clientes por Faturamento</div>
                    <div className="chart-card-body">
                      <div className="table-responsive">
                        <table className="table table-dark table-sm align-middle mb-0">
                          <thead>
                ...
                <div className="col-12 col-lg-6">
                  <div className="chart-card">
                    <div className="chart-card-header">Ranking de Usuários</div>
                    <div className="chart-card-body">
                      <div className="table-responsive">
                        <table className="table table-dark table-sm align-middle mb-0">
                          <thead>
                          ...
                          <tbody>
                            {(stats.topUsers || []).map((u, i) => (
                              <tr key={i}>
                                ...
                                <td className="text-center small">{u.order_count}</td>
                                <td className="text-end small">R$ {parseFloat(u.total_value).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                ...
                <div className="col-12 col-lg-6">
                  <div className="chart-card">
                    <div className="chart-card-header d-flex justify-content-between align-items-center">
                      ...
                      <button className="btn btn-outline-secondary btn-sm" style={{ fontSize: "0.72rem" }} onClick={() => navigate("/admin/users")}>Gerenciar</button>
                    </div>
                    <div className="chart-card-body">
                      <div className="table-responsive">
                        <table className="table table-dark table-sm align-middle mb-0">
                          ...
                          <tbody>
                            {(stats.recentUsers || []).map((u) => (
                              <tr key={u.id} role="button" onClick={() => navigate(`/admin/users/${u.id}/edit`)}>
                                <td className="small">{u.id}</td>
                                <td className="small">{u.name}</td>
                                <td className="text-secondary small">{u.email}</td>
                                <td><span className={`badge bg-${u.role === "admin" ? "danger" : "primary"}`} style={{ fontSize: "0.63rem" }}>{u.role}</span></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
